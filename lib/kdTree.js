...
KDTree.prototype.minimum = function (root, dimension) {
	...
	if (dimension == alignment) {
		if (root.left == null) {
			...
			this.minimum (root.left, dimension);
		}
	}
	else {
		var left = Infinity, right = Infinity;

		if (root.left != null) { left = this.minimum (root.left, dimension); }
		if (root.right != null) {right = this.minimum (root.right, dimension); }

		return (Math.min (root.keyArray [dimension], left, right));
	}
};
